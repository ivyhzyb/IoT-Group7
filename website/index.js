...
AWS.config.credentials.get(function (err, data) {
        ...
        cognitoIdentity.getCredentialsForIdentity(params, function (err, data) {
            if (!err) {
                ...
                thingShadows.on('status', function (thingName, statusType, clientToken, stateObject) {
                    ...
                    if (statusType === 'rejected') {
                        ...
                        if (stateObject.state.hasOwnProperty('reported') && stateObject.state.reported.hasOwnProperty('Armed_Status')) {
                            ...
                            handleTruckImage(stateObject.state.reported.Armed_Status);
                        }
                    }
                });
                ...
                TruckLightsEventButton.addEventListener('click', (evt) => {
                    thingShadows.update(thingName, { state: { reported: { Armed_Status: !(currentArmedStatus) } } });
                });
 ...
 // Function handling the truck image based on the newArmedStatus
function handleTruckImage(newArmedStatus) {

    // Don't do anything if the light status hasn't changed
    if (currentArmedStatus === newArmedStatus) {
        return;
    } else if (newArmedStatus === true) {
        console.log('changing truck image to Armed ON');
